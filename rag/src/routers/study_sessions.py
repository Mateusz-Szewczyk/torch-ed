...
def create_study_session(
    ...
    # Szukamy Deck, który należy do zalogowanego użytkownika (user_id typu int)
    ...
    # Inicjalizacja UserFlashcard dla każdej fiszki w tym decku, jeśli jeszcze nie istnieje
...
def get_next_flashcard(
    ...
    # Pobierz wszystkie user_flashcards w Deck powiązanym z tą sesją
    # i filtruj te, które są do przeglądu (next_review <= today).
    today = datetime.utcnow()
    deck = session.deck  # Relacja w modelu
    if not deck:
    ...
    flashcard = db.query(Flashcard).filter(
        Flashcard.id == user_flashcard.flashcard_id
    ).first()

    if not flashcard:
        logger.error(
            f"Fiszka z ID={user_flashcard.flashcard_id} nie istnieje w bazie."
        )
...
def record_flashcard_review(
    ...
    if not user_flashcard:
        logger.error(
            f"UserFlashcard z ID={record_create.user_flashcard_id} nie znaleziony "
            f"w decku ID={deck.id} (sesja={session_id})."
        )
        raise HTTPException(status_code=404, detail="Fiszka nie znaleziona.")
    ...
    user_flashcard.ef += (
        0.1
        - (5 - record_create.rating) * (0.08 + (5 - record_create.rating) * 0.02)
    )
    ...
    logger.info(f"Zaktualizowano dane SM2 dla user_flashcard ID={user_flashcard.id}")
    return study_record
