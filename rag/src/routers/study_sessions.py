...
from ..schemas import StudySessionCreate, StudySessionRead, StudyRecordCreate, StudyRecordRead
...
def create_study_session(
    ...
    deck = db.query(Deck).filter(Deck.id == session_create.deck_id, Deck.user_id == str(current_user.id_)).first()
    if not deck:
    ...
    user_flashcards = db.query(UserFlashcard).filter(UserFlashcard.user_id == str(current_user.id_),
                                                     UserFlashcard.flashcard_id.in_(
                                                         [fc.id for fc in deck.flashcards])).all()
    existing_flashcard_ids = set(uf.flashcard_id for uf in user_flashcards)
    new_flashcards = []
...
@router.get("/next_flashcard/{session_id}/", response_model=Flashcard)
def get_next_flashcard(
    ...
    session = db.query(StudySession).filter(StudySession.id == session_id,
                                            StudySession.user_id == str(current_user.id_)).first()
    if not session:
...
def record_flashcard_review(
    ...
    """
    logger.info(
        f"Rejestrowanie oceny dla sesji_id={session_id}, user_flashcard_id={record_create.user_flashcard_id}, rating={record_create.rating} przez user_id={current_user.id_}")
    ...
    user_flashcard = db.query(UserFlashcard).filter(UserFlashcard.id == record_create.user_flashcard_id,
                                                    UserFlashcard.user_id == str(current_user.id_),
                                                    UserFlashcard.flashcard_id.in_(
                                                        [fc.id for fc in session.deck.flashcards])).first()
    if not user_flashcard:
        logger.error(
